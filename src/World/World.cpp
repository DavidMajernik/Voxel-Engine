...
World::World() {
	...
	chunkPos = glm::ivec2(0.0f, 0.0f); // Initialize chunk position to (0, 0)
...
void World::updateChunks(glm::vec3 camPos)
{
    ...
    chunkPos = glm::ivec2(
		static_cast<int>(camPos.x / Chunk::SIZE),
		static_cast<int>(camPos.z / Chunk::SIZE)
	);
	//std::cout << "Camera chunk position: " << chunkPos.x << ", " << chunkPos.y << std::endl;
	...
	for (int x = -renderDistance; x <= renderDistance; x++)
	{
		for (int z = -renderDistance; z <= renderDistance; z++)
		{
			...
			{
				// If it doesn't exist, create a new chunk at the calculated position
				futureChunkMap[chunkKey] = std::async(std::launch::async, [chunkKey]() {
					...
					Chunk temp(glm::vec3(chunkKey.x * Chunk::SIZE, 0.0f, chunkKey.y * Chunk::SIZE));
					return temp;
					});
	...
	while (!chunkBuildQueue.empty() && chunksBuilt < maxChunksPerFrame)
	{
		...
		std::cout << "Building chunk at: " << chunk.position.x << ", " << chunk.position.z << std::endl;
		glm::ivec2 key = glm::ivec2(chunk.position.x / Chunk::SIZE, chunk.position.z / Chunk::SIZE);
